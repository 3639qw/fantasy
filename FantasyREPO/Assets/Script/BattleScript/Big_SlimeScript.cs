...
public class Big_SlimeScript : MonoBehaviour, IDamageable
{
    ...
    private BigSlimeState currentState;
    ...
    private void Start()
    {
        ChangeState(BigSlimeState.Idle);
    }
    ...
    private void Update()
    {
        ...
        switch (currentState)
        {
            case BigSlimeState.Idle:
                if (distanceToPlayer < chaseRange)
                {
                    ChangeState(BigSlimeState.Chase);
                }
                else if (stateTimer <= 0f)
                {
                    ChangeState(BigSlimeState.Wander);
                }
                break;

            case BigSlimeState.Wander:
                if (distanceToPlayer < chaseRange)
                {
                    ChangeState(BigSlimeState.Chase);
                }
            ...
            case BigSlimeState.Attacked:
                if (stateTimer <= 0f)
                {
                    ChangeState(distanceToPlayer > chaseRange ? BigSlimeState.Idle : BigSlimeState.Chase);
                }
                break;

            case BigSlimeState.Die:
                if (stateTimer <= 0f)
                {
                    Destroy(gameObject);
                }
    ...
    private void ChangeState(BigSlimeState newState)
    {
        ...
        {
            case BigSlimeState.Idle:
                animator.Play("Big_Red_Slime_Idle");
                path.ClearCorners();
                break;

            case BigSlimeState.Wander:
                animator.Play("Big_Red_Slime_Jump");
            ...
            case BigSlimeState.Attacked:
                animator.Play("Big_Red_Slime_Attacked");
                stateTimer = 0.3f;
                break;

            case BigSlimeState.Die:
                animator.Play("Big_Red_Slime_Die");
                stateTimer = 1.2f;
                path.ClearCorners();
    ...
    public void TakeDamage(float amount)
    {
        ...
        if (MonsterHP <= 0f)
        {
            ChangeState(BigSlimeState.Die);
            Debug.Log($"{gameObject.name} 사망!");
        }
        else if (currentState != BigSlimeState.Attacked)
        {
            ChangeState(BigSlimeState.Attacked);
        }
    }
}
